...
SELECT * FROM salespeople WHERE region IN ('Northwest')
...
ELECT email FROM salespeople WHERE region IN ('Northwest', 'Southwest') AND last_name LIKE 'M%';
...
SELECT melon_type, common_name, price,  price * .73 AS euroMelons FROM melons ORDER by euroMelons;
...
SELECT id FROM customers WHERE email = 'pclark74@gmial.com'

-- Problem 20:
-- Task: Write a query that shows the id, status and order_total for all
-- orders made by customer 100.

quiz=# SELECT id, status, order_total FROM orders  WHERE id = 100;
